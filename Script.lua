...
local AncestorCheck = if script.Parent:IsA("ModuleScript") then script.Parent.Parent.Parent elseif script.Parent:IsA("Model") then script.Parent  else script.Parent.Parent
--if not AncestorCheck:IsA("Player") then script.Disabled=true;return;end
local Player,Mouse,mouse,UserInputService,ContextActionService = AncestorCheck,nil,nil,nil,nil
local ShiftLock = false
local ClientCFrame = nil
local VectorLook = nil
if AncestorCheck:IsA("Player") then
	--if Services.RunService:IsStudio() then Player.CharacterAppearanceId = 682618485 end
	--require(Modules.CharacterModule)("R6",Player.Name,Player.CharacterAppearanceId);
	--task.wait(3)
	do
		local GUID = {}
		do
...
Clerp = function(a,b,t) 
	local ClerpFunctions = require(Modules.ClerpFunctions);
...
local AnimModule = require(Modules.Animator)(Services,Character,Body,AVt,ACf)
